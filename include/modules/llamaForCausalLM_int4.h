...
struct Int4LlamaForCausalLM_input {
    ...
    Int4LlamaForCausalLM_input() = default;
    explicit Int4LlamaForCausalLM_input(Matrix3D<int> input_ids_) : input_ids(input_ids_) { has_past_keys_values = false; }
...
class Int4LlamaForCausalLM {
   public:
    Int4LlamaForCausalLM(std::string param_path, struct model_config config);
